    ...
    <div className="glass-panel p-5 space-y-4">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-100 text-sm tracking-wide">Face Verification Module</h3>
        </div>
        <Badge label={face.status.replace(/_/g, ' ')} variant={variant} />
      </div>

      <div className="flex items-center justify-between bg-navy-950/60 p-4 rounded-xl border border-slate-800">
        <div>
          <p className="text-[11px] font-mono text-slate-400 uppercase">Cosine Similarity Score</p>
          <p className={`text-2xl font-bold font-mono mt-0.5 ${isMatched ? 'text-emerald-400' : 'text-amber-400'}`}>
            {(face.similarity * 100).toFixed(0)}% <span className="text-xs text-slate-500 font-normal">Match</span>
          </p>
        </div>
        <div>
          <p className="text-[11px] font-mono text-slate-400 uppercase">Detection Status</p>
      ...
      <p className="text-xs text-slate-400 leading-relaxed font-sans bg-slate-800/40 p-3 rounded-lg border border-slate-800">
        {face.details}
      </p>
    </div>
  );
