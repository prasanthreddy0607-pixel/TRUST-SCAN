    ...
    <div className="glass-panel p-5 space-y-4">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-slate-100 text-sm tracking-wide">OCR & MRZ Extracted Field Data</h3>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {Object.entries(ocr.fields).map(([key, item]) => {
          ...
          return (
            <div key={key} className="bg-navy-950/60 p-3 rounded-lg border border-slate-800 space-y-1">
              <div className="flex items-center justify-between text-[11px] font-mono text-slate-400">
                <span className="uppercase">{key.replace(/_/g, ' ')}</span>
                <span className="text-slate-500">{Math.round(item.confidence * 100)}% Conf</span>
              </div>
              <p className="font-bold text-slate-100 text-sm font-mono truncate">{item.value || 'N/A'}</p>

              {mrzComp && (
                <div className={`text-[10px] font-mono flex items-center gap-1 mt-1 pt-1 border-t border-slate-800/60 ${isConsistent ? 'text-emerald-400' : 'text-red-400 font-bold'}`}>
                  {isConsistent ? (
                    <>
      ...
      {mrz && mrz.detected && mrz.raw_mrz.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-slate-800">
          ...
          <div className="bg-navy-950 p-3 rounded-lg border border-slate-800 font-mono text-xs text-cyan-400 leading-relaxed tracking-wider space-y-1 select-all">
