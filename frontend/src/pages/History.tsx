          ...
          <div className="glass-panel overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-800 bg-navy-950/60 font-mono text-slate-400 text-[11px] uppercase">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {screenings.length === 0 ? (
                    ...
                    screenings.map((item) => {
                      ...
                      return (
                        <tr key={item.screening_id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-3.5 pl-5 font-mono font-medium text-slate-200">{item.screening_id}</td>
                          <td className="p-3.5 text-slate-300 font-semibold uppercase">{item.document_type}</td>
                          ...
                          <td className="p-3.5 text-right pr-5 space-x-2">
                            <button
                              ...
                              className="p-1.5 rounded bg-slate-800 hover:bg-slate-700 text-blue-400 border border-slate-700 transition-colors inline-flex items-center gap-1 text-[11px]"
                            >
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-1.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 transition-colors inline-flex items-center gap-1 text-[11px]"
                            >
                              <FileText className="w-3.5 h-3.5" /> PDF
                            </a>
                            <button
                              onClick={() => handleDelete(item.screening_id)}
                              className="p-1.5 rounded bg-slate-800 hover:bg-red-900/40 text-red-400 border border-slate-700 transition-colors"
                            >
